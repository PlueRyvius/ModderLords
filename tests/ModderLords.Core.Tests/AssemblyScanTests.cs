...
public sealed class AssemblyScanTests
{
    ...
    {
        var r = ScanSelf();
        Assert.Contains("ModderLords.Core.Tests.FakeModSettings (4 values)", r.SettingsClasses);   // Enabled, Max, Rate, Name
        Assert.Contains("ModderLords.Core.Tests.FakeStaticConfig (2 values)", r.SettingsClasses);  // Count, On (Const is readonly)
        Assert.Contains("ModderLords.Core.Tests.FakePlain (2 values)", r.SettingsClasses);         // singleton Current + Value, Mode(enum)
    }
    ...
    {
        var r = ScanSelf();
        Assert.DoesNotContain(r.SettingsClasses, s => s.StartsWith("ModderLords.Core.Tests.FakeSettingsVM"));
        Assert.DoesNotContain(r.SettingsClasses, s => s.StartsWith("ModderLords.Core.Tests.FakeThing"));
        Assert.DoesNotContain(r.SettingsClasses, s => s.StartsWith("ModderLords.Core.Tests.FakeTemplateSettings"));
        Assert.DoesNotContain(r.SettingsClasses, s => s.StartsWith("ModderLords.Core.Tests.FakeManager"));
        Assert.DoesNotContain(r.SettingsClasses, s => s.StartsWith("ModderLords.Core.Tests.FakeUIManager"));      // singleton-only + Manager suffix
        Assert.DoesNotContain(r.SettingsClasses, s => s.StartsWith("ModderLords.Core.Tests.FakeOrphanSettings"));  // unreachable
        ...
        Assert.Contains("ModderLords.Core.Tests.FakeMcmSettings (1 value)", r.SettingsClasses);   // its generic base is local, not from an MCM assembly
    ...
    public void Recipes_CarrySettingsHints_ForUngatedMods()
    {
        ...
        var mods = ModderLords.CompatSync.MiniJson.GetArray(root, "Mods")!;
        var first = (Dictionary<string, object?>)mods[0]!;
        Assert.NotNull(ModderLords.CompatSync.MiniJson.GetObject(first, "Settings"));
    }
}
