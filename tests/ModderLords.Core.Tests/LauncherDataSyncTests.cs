...
public class LauncherDataSyncTests : IDisposable
{
    ...
    public void Server_only_modules_are_ignored()
    {
        ...
        var plan = LauncherDataSync.ComputePlan(
            [E("DedicatedServer.ModderLordsCompat", "v0.1.0"), E("DedicatedServer.Windows", "v1.4.8")],
            Order("DedicatedServer.ModderLordsCompat"), path);
    ...
    {
        var path = WriteFile(Mod("CoopNightly", "v0.1.4", true), Mod("ModderLords.Compat", "v0.1.0", true));
        var installed = new HashSet<string>(StringComparer.OrdinalIgnoreCase);   // nothing on disk any more
        var plan = LauncherDataSync.ComputePlan([E("ModderLords.Compat", "v0.1.0")], Order("ModderLords.Compat"), path, installed);
        Assert.Contains(plan.Blockers, c => c.Id == "ModderLords.Compat" && c.Detail.Contains("not on this PC any more"));
    }
