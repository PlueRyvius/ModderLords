...
/// <summary>Diagnostic: validates the bundled ModderLords.Compat manifest the way the BUTR launcher does, against the local game install.</summary>
public class ManifestValidationProbe
{
    ...
    public void Compat_manifest_validates_against_local_game_modules()
    {
        ...
        var target = mods.FirstOrDefault(m => m.Id == "ModderLords.Compat");
        if (target is null) { _out.WriteLine("ModderLords.Compat not installed in game Modules; skipped"); return; }
        ...
        var selected = new HashSet<string>(new[] { "Bannerlord.Harmony", "Bannerlord.UIExtenderEx", "Bannerlord.ButterLib", "Bannerlord.MBOptionScreen", "Native", "SandBoxCore", "CustomBattle", "Sandbox", "StoryMode", "ModderLords.Compat" });
