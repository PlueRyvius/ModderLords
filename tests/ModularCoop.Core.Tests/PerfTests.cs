...
public class PerfLineParserTests
{
    ...
    private const string Line = "[ModularCoop] perf tick avgFps=142.7 minFps=38.2 frames=1427 windowSec=10.0 campaignMode=Real";
    ...
    {
        var s = PerfLineParser.TryParse("[ModularCoop] perf players count=3", At);
    ...
    [InlineData("")]
    [InlineData("   ")]
    [InlineData("[ModularCoop] guards installed: 9")]          // our own Tool line, not perf
    [InlineData("[ModularCoop.Compat] something")]
    [InlineData("[Fps] frames=100 avg=50")]                     // Coop's own line: not ours to parse
    [InlineData("[ModularCoop] perf ")]
    ...
    public void A_truncated_or_malformed_line_loses_fields_rather_than_throwing()
    {
        ...
        var s = PerfLineParser.TryParse("[ModularCoop] perf tick avgFps=142.7 minFps= frames junk =x windowSec=abc", At);
        Assert.NotNull(s);
        Assert.Equal(142.7, s!.Number("avgFps"));
    ...
    {
        var s = PerfLineParser.TryParse("[ModularCoop] perf memory bytes=42", At);
        Assert.Equal(PerfKind.Unknown, s!.Kind);
        Assert.Equal(42, s.Count("bytes"));
    }

    /// <summary>
    /// The classifier is order-dependent: the generic "[ModularCoop]" branch would swallow perf lines into Tool if
    ...
    public void Perf_lines_classify_as_Perf_not_Tool()
    {
        ...
        Assert.Equal(LogCategory.Tool, LogClassifier.Classify("[ModularCoop] mod list already matches the server").Category);
    }
}
