...
{
    /// <summary>Set to 0 to silence the comparison. It costs one read, once.</summary>
    ...
    internal static void Tick()
    {
        ...
        try
        {
            ...
            Log.Info($"map bounds derived from the scene : {derivedMin}..{derivedMax} height={derivedHeight:0.###} terrain={derivedTerrain}");
            Log.Info($"map bounds currently in effect    : {liveMin}..{liveMax} height={liveHeight:0.###} terrain={liveTerrain}");
            var agrees = Near(derivedMin, liveMin) && Near(derivedMax, liveMax)
                         && Math.Abs(derivedHeight - liveHeight) < 0.01f && Near(derivedTerrain, liveTerrain);
            Log.Info(agrees
                ? "map bounds: the scene agrees with the injected values, so the map sidecar is redundant here"
                : "map bounds: WARNING the scene and the injected values disagree; the sidecar is still doing work");
        }
        catch (Exception ex)
        {
            _done = true;
